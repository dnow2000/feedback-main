...
if [[ $# -gt 2 ]] && [[ "$1" == "-b" ]]; then
  ...
  BACKUP_PATH=~/${APP_NAME}-main/db_prod_dumps/
...
PGPASSWORD="$PG_PASSWORD" pg_dump --host 127.0.0.1 --port 10000 --no-comments --no-owner --schema-only --exclude-table=alembic_version --username "$PG_USER" --dbname "$PG_USER" --format c > $FILE
