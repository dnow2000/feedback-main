...
def create_scenes(with_capture=False):
    logger.info('create_scenes')
    ...
    scenes.append(Scene(
        ...
        url='https://www.lemonde.fr/sciences/scene/2018/07/24/maladie-de-lyme-fronde-contre-la-haute-autorite-de-sante_5335369_1650684.html'
    ))
    ...
    scenes.append(Scene(
        ...
        url='https://www.lemonde.fr/cinema/scene/2019/01/10/cocorico-fred-poulet-revient-a-la-chanson_5407141_3476.html'
    ))

    ApiHandler.save(*scenes)

    if with_capture:
        for scene in scenes:
            logger.info('capture screenshot for {}...'.format(scene.url))
            thumb = capture(scene.url)
            save_thumb(scene, thumb, 0)
            logger.info('capture screenshot for {}...Done.'.format(scene.url))

    logger.info('created {} scenes'.format(len(scenes)))
