...
BASE_URL = f'https://www.googleapis.com/customsearch/v1?key={API_KEY}&cx={ENGINE_ID}&q='


def backlinks_from_url(url, count=10, page=1):
    backlinks = []
    parsed = urllib.parse.urlparse(url)
    exact_terms = f'{parsed.netloc}{parsed.path}'
    if page > 0:
        start = ((page - 1) * count) + 1
        results = requests.get(f'{BASE_URL}{url}&exactTerms={exact_terms}&count={count}&start={start}').json()
        items = results['items']
